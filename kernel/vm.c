...
void 
kernel_freewalk(pagetable_t pagetable) 
{
  for(int i = 0; i < 512; i++){
...
int
kvmcopy(pagetable_t old, pagetable_t new)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = 0; i < 512; i++){
    if((pte = walk(old, i*PGSIZE, 0)) == 0 || (*pte & PTE_V) == 0)
      continue;
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    if((mem = kalloc()) == 0)
      goto err;
    memmove(mem, (char*)pa, PGSIZE);
    if(mappages(new, i*PGSIZE, PGSIZE, (uint64)mem, flags) != 0){
      kfree(mem);
      goto err;
    }
  }
...
void
copy_page_table(pagetable_t old, pagetable_t new, uint64 start, uint64 end)
{
  pte_t *ptesrc, *ptedst;
  uint64 i;
  if(start > end) return;
...
void
kvmdealloc(pagetable_t pagetable, uint64 start, uint64 end)
{
  if(end >= start) return;
